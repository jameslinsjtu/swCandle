...
//    dma_set_stepsize(&gva1,40);
	...
	start = total_size/64*id;
	end = total_size/64*(id+1);
//		simd_load(num1,f1_t);

     if(op==3)//triad
    {
 //     for(i=total_size/64*id;i<total_size/64*(id+1);i+=4)
...
*/
		...
		simd_load(num2,f2_t+i);
		simd_load(num3,f3_t+i);
		num1 = num2+sc*num3;
		simd_store(num1,f1_t+i);
    ...
    else if(op==0)//copy
    {
      ...
      {/*
		...
		*/
		...
		simd_load(num1,f1_t+i);
		simd_store(num1,f2_t+i);
//		f1_t[i] = f2_t[i];
      }
    }
