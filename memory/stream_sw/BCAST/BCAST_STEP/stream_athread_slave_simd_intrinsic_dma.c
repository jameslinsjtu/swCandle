...
//#define buf_size 1//2//4//8//16//32//64//128//256//512//1024//2048//3072//3500//3125//3072//3200
#define REAL double
//#define STEP_SIZE	32
...
void fun(struct parameter* para)
{
    ...
    dma_set_mode(&gva,BCAST_MODE);
    dma_set_size(&gva,buf_size*sizeof(double));
    dma_set_mask(&gva,255);
//    dma_set_bsize(&gva,BSIZE*sizeof(double));
//    dma_set_stepsize(&gva,STEP_SIZE*sizeof(double));
    dma_set_reply(&gva,&hs_reply);

    dma_set_op(&gva1,DMA_PUT);
    dma_set_mode(&gva1,PE_MODE);
    dma_set_bsize(&gva1,BSIZE*sizeof(double));
    dma_set_stepsize(&gva1,STEP_SIZE*sizeof(double));
    dma_set_size(&gva1,buf_size*sizeof(double));
    ...
    if(op==0)//copy
    {
      for(i=total_size/64*id;i<total_size/64*(id+1);i+=buf_size/BSIZE*(BSIZE+STEP_SIZE))
      {
if(id==0){
        hs_reply=0;
        hs_reply1=0;
