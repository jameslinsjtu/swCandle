...
#define buf_size BSIZE//2//4//8//16//32//64//128//256//512//1024//2048//3072//3500//3125//3072//3200
#define REAL double
struct parameter
{
    double * f1;
...
void fun(struct parameter* para)
{
    ...
    dma_set_bsize(&gva,16*8);
    dma_set_stepsize(&gva,4);
...
//    dma_set_stepsize(&gva1,40);
    ...
    if(op==0)//copy
    {
      for(i=total_size/64*id;i<total_size/64*(id+1);i+=buf_size)
      {
        ...
        hs_reply=0;
        hs_reply1=0;
        dma(gva,f2_t+(i%(total_size/64)),ldm);
        dma_wait(&hs_reply,1);}
//        dma(gva1,f1_t+i,ldm);
